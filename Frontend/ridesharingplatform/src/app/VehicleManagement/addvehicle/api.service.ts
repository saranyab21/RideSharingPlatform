...
export class VehicleApiService {
  ...
  createVehicle(applicationData: any): Observable<any> {
    ...
    console.log("ht " + this.token);
    const headers = new HttpHeaders({ Authorization: 'Bearer ' + this.token });
    return this.http.post<any>(`${this.apiUrl}/vehicles/addvehicle`, applicationData, { headers })
      .pipe(
        catchError(this.handleError)
      );
  }
  ...
  getPendingVehicles(): Observable<any> {
    ...
    console.log("ht " + this.token);
    const headers = new HttpHeaders({ Authorization: 'Bearer ' + this.token });
    return this.http.get<any>(`${this.apiUrl}/vehicles/pendingapprovals`, { headers })
      .pipe(
        catchError(this.handleError)
      );
  }
  ...
  deleteVehicle(regno: string): Observable<any> {
    ...
    return this.http.delete(`${this.apiUrl}/vehicles/${regno}`, { headers })
      .pipe(
        catchError((error) => {
          return throwError(() => new Error(error));
        })
